...
function storeMemesInLocalStorage(memeLinks){

    const links = [{imgLink:memeLinks}];
    const storedMemes = JSON.stringify(links);
    localStorage.setItem('UserMemes', storedMemes);
...
memePageForm.addEventListener("submit", function(event){
    ...
    const newMeme = userMeme(imgLink.value, topMsg.value, btmMsg.value);
            ...
            storeMemesInLocalStorage(imgLink.value);
